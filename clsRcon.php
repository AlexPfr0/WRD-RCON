...
class clsRcon {
    ...
    protected function rawPacketRead() {
        ...
        $sBuffer = '';
        while (socket_select($aRead, $aWrite, $aExcept, 0, $this->m_iReadTimeout)) {
 ...
 * test case below
 */
//$commande = $_REQUEST["name"];
$value = str_replace("="," ",$_REQUEST["value"]);
$ip =   $_COOKIE['wrd_srv_ip'];
$port = $_COOKIE['wrd_srv_port'];
$mdp =  $_COOKIE['wrd_srv_mdp'];
$oTest = new clsRcon($ip, $port, $mdp);
$oTest->connect();

$aResponse = $oTest->rcon($value);


if (isset($aResponse[0])) {
    echo($aResponse[0]['String1']);
